...
class FreeColorLidarSensor(RawSensor):
    ...
    def __init__(self,
        ...
        super(FreeColorLidarSensor, self).__init__(
    ...
    def read(self, state, cube_params):
        ...
        if torch.any(torch.isnan(depth)):
            print("Nan in depth found at: ", state)
        
        if torch.any(torch.isnan(im)):
            print("Nan in rgb found at: ", state)
