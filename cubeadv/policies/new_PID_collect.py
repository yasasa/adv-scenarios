...
now = dt.datetime.now()
out_dir = 'experiments/%d-%d-%d' % (now.year, now.month, now.day)
...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            # self.world.set_weather(self._weather_presets[0][0])
    ...
    def parse_image_custom(self, surface, image, sensor_name, save):
        pass
...
def clean_lidar(img_lidar):
    ...
    # print("tst_inputs shape is: ", tst_inputs.shape)
    # tst_inputs /= 50.
    # assert if there's large points
    if (tst_inputs.shape[0] != 0):
        assert(np.max(tst_inputs) < np.sqrt(60.5**2 * 60.5**2))
    else:
        print("size of tst_inputs is 0")
...
def game_loop(args):
    ...
    try:
        ...
        keyboard_manual = False
        if not keyboard_manual:
        ...
        ## training road
        if keyboard_manual:
            # world.player.set_transform(carla.Transform(carla.Location(x=250.0, y=260.0, z=2.0), carla.Rotation(pitch=0.0, yaw=180.0, roll=0.0)))

            ## Verify that the colour mapping from image to lidar point cloud is correct
            # world.player.set_transform(carla.Transform(carla.Location(x=77.0, y=87.4, z=2.0), carla.Rotation(pitch=0.0, yaw=180.0, roll=0.0)))

            ## Verify that the new cubes added are correct
            world.player.set_transform(carla.Transform(carla.Location(x=90.0, y=100.0, z=2.0), carla.Rotation(pitch=0.0, yaw=90.0, roll=0.0)))
        else:
        ...
        with world:
            ...
            while True:
                ...
                if ret:
                    ...
                    points = clean_lidar(img_lidar) # n x 3
                    images_clean = [clean_img(img_rgb), clean_img(img_rgb_1), clean_img(img_rgb_2), clean_img(img_rgb_3)] # 1280 x 720 x 3

                    if add_cubes:
                        # Get the return info about the cubes that were hit
                        # For now: 2 actors {'0': [fid, fid], '1': [fid, fid]}
                        hit_cube_info.clear()
                        # hit_cube_info = get_building_info(img_lidar)
                        # print(hit_cube_info)
                    
                    # world.lidar_saver.add_image(points, 'Lidar') # n x 3
                    # world.rgb_saver.add_image(images_clean[0], 'CameraRGB_0') # 1280 x 720 x 3
                    # world.rgb_saver_1.add_image(images_clean[1], 'CameraRGB_1') # 1280 x 720 x 3
                    # world.rgb_saver_2.add_image(images_clean[2], 'CameraRGB_2') # 1280 x 720 x 3
                    # world.rgb_saver_3.add_image(images_clean[3], 'CameraRGB_3') # 1280 x 720 x 3
                    ...
                    # get only the RGBD data from the processed point data (n x 9)
