...
def game_loop(args):
    ...
    iteration = 0
    os.mkdir('%s/ep_%d/' % (out_dir, iteration))
    ...
    try:
        ...
        start_location = carla.Location(x=120.0, y=129.0, z=2.0)
        # Default training road:
        # start_location = carla.Location(x=305.0, y=129.0, z=2.0)

        world.player.set_simulate_physics(False)
        world.player.set_transform(carla.Transform(start_location, start_rotation))
        
        saver_position = BufferedImageSaver('%s/ep_%d/' % (out_dir, iteration),
                       100, 1, 1, 3, output_name)
        position = []

        # Spawn cubes
        blueprintsBuildings = carla_world.get_blueprint_library().filter(args.filterb)
        ...
        spawn_points = []
        spawn_loc_file = "random_sampling/spawn_points.txt"
        transform_info = get_spawn_points_from_file(spawn_loc_file)
        ...
        actors = carla_world.get_actors(building_list)
        # actors = carla_world.get_actors().filter("building.mybuilding.cube")
        print(actors)
        ...
        fout = open(out_dir+"/ep_"+str(iteration)+"/buildings.txt", 'w')
        for act in actors:
            print(act, file=fout)
        fout.close()
        fout = open(out_dir+"/ep_"+str(iteration)+"/spawned.txt", 'w')
        ...
        print('random_sample_v2: current location, ', world.player.get_location())

        while world.player.get_location() != start_location:
            print("Player spawn location has not been updated. Try again.")
            world.player.set_transform(carla.Transform(start_location, start_rotation))
            clock.tick()
            ret = world.tick(timeout=10.0)
        ...
        with world:
            ...
            while True:
                ...
                if len(position) == 350:
                    ...
                    # Respawn buildings to different locations
                    ...
                    print('random_sample_v2: iter: ', iteration, ' , current location, ', world.player.get_location())
                    agent.reset()
...
def main():
    ...
    argparser.add_argument(
        ...
        default=True,
        type=bool,
        help='whether run with cubes spawned (default: True)')
    argparser.add_argument(
        '--maxIter',
        default=1,
        type=int,
        help='maximum number of iterations to sample')
    argparser.add_argument(
        '--filterb',
        metavar='PATTERN',
    ...
    if os.path.exists(out_dir):
        print("The experiment directory already exists. To avoid overriding previous data, aborting now.")
        assert(0)
    else:
        os.mkdir(out_dir)
