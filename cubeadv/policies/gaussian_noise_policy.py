...
def game_loop(args):
    ...
    try:
        ...
        saver_position = BufferedImageSaver('%s/ep_%d/' % (out_dir, episode_count),
                               100, 1, 1, 3, 'Position')
        ...
        with world:
            ...
            while True:
                ...
                if ret:
                    ...
                    noise_rgb = np.random.normal(0.5, 0.5**2, 32*100*3).reshape(3,32,100)
                    noise_d = np.random.normal(30, 3**2, 32*100).reshape(1,32,100)
                    curr_input[0:3] = curr_input[0:3] + noise_rgb
                    curr_input[3] = curr_input[3] + noise_d
