...
PWD = '/home/siyun/CARLA/repo/carla/Dist/CARLA_Shipping_0.9.6-14-gab813b69-dirty/LinuxNoEditor/PythonAPI'
########################################################################################
### Set all the local environment variables here
folder = '/1001'
##CNN
from CNN import Net
...
class World(object):
    ...
    def restart(self):
        ...
        ## Store data in ./dagger_data/ep_#/xxx
    ...
    def on_collision(weak_self, event):
        ## Set collision_glb to True if collision occurred
...
def get_building_info(img_lidar):
    ## Get the actor and face ids that was hit
    actor_dict = {}
    actor_dict.clear()
    num_actors = img_lidar.actors
    face_id_total_list = np.frombuffer(img_lidar.raw_data_actor_face_ids, dtype=np.dtype('uint32'))
    actor_name_string = bytes(img_lidar.raw_data_actor_names).decode('UTF-8')
    name_idx, face_id_idx = 0, 0
    for i in range(num_actors):
        a_name_len = img_lidar.get_actor_name_string_length(i)
        a_name = actor_name_string[name_idx:name_idx+a_name_len]
        fid_len = img_lidar.get_actor_face_id_count(i)
        actor_dict[a_name] = face_id_total_list[face_id_idx:face_id_idx+fid_len]
        name_idx += a_name_len
        face_id_idx += fid_len
    return actor_dict  # 2 actors {'0': [fid, fid], '1': [fid, fid]}
...
def game_loop(args):
    ...
    if load_pretrained:
        weights_path = ('./model/dagger_%d.pth' % episode_count)
        ...
        with world:
            ...
            while True:
                ...
                if ret:
                    ...
                    # curr_input = combine_cnn_lidar_data(points)
                    curr_input = colour_lidar(points, images_clean)  # n x 6
                    validate_lidar_colour.add_image(curr_input, 'Coloured_Lidar')

                    curr_input = lidar_project2d(curr_input)  # 3200 x 9
                    # validate_all.add_image(get_all_data(curr_input), 'All')
