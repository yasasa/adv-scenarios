...
# Based on the script from https://github.com/NVlabs/instant-ngp/tree/master/scripts
...
def parse_args():
    ...
    parser.add_argument("--image_folder", default=".", help="nsvf dataset path")
...
if __name__ == "__main__":
    ...
    #print ("WARNING COMMENTED OUT BBOX")
    ...
    num_list = " ".join(lines).split(" ")
    num_list = [x for x in num_list if x]
    els = tuple(map(float, num_list))
    ...
    for itype in [0,1,2]:
            ...
            for img_f in img_files[itype]:
                    ...
                    lines = open(pose_f).readlines()
                    num_list = " ".join(lines).split(" ")
                    num_list = [x for x in num_list if x]
                    elems = tuple(map(float, num_list))
